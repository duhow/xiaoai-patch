...
for FILE in alarmd bluez_mibt_ble mibt_ble bluez_mibt_classical carrier_chinatelecom.sh carrier.sh mediaplayer messagingagent mdplay \
  miplayer mibrain_level mibrain_net_check mibrain_oauth_manager mibrain_service mibt_mesh mibt_mesh_proxy \
  mipns-xiaomi mipns-sai mipns-horizon \
  mico_ai_crontab mico_vendor_helper mico_model_helper \
  heartbeatagent ciptool xiaomi_dns_server \
...
for FILE in libmibrain-common-sdk.so libmibrain-common-util.so libmibrainsdk.so \
	...
	libDiracAPI_SHARED.so libdts.so libxaudio_engine.so libmesh.so libquark_lib.so libvpm.so \
...
for NAME in xiaomi sai horizon; do
